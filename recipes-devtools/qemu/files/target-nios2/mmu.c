...
void mmu_init (CPUNios2State *env)
{
    ...
    /* Initialize MMU values to typical (recommended) values.
     * The values can be modified later on at runtime.
     * If MMU is not used, the values are not needed.
     */
    env->mmu.pid_bits = 8;
    env->mmu.tlb_num_ways = 16;
    env->mmu.tlb_num_entries = 256;

    if (cpu->mmu_present) {
        env->mmu.tlb = (Nios2TLBEntry *)g_malloc0(
                    sizeof(Nios2TLBEntry) * env->mmu.tlb_num_entries);
    } else {
        env->mmu.tlb = NULL;
        env->mmu.tlb_entry_mask = 0;
    }
}
